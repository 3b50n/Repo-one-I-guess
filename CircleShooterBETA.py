...
enemies = []
max_enemies = 5 # maximum number of enemies allowed on the screen at once
...
while running:
    ...
    #get the x and y position of the right joystick
    x2_axis = joystick.get_axis(2)
    ...
    # Check if the A button is pressed/ now supposed to check for the joystick
    if x2_axis or y2_axis:
        ...
        if len(bullets) < max_bullets:
            ...
            bullspeedlist.append(bullet["dir"])
    

    # Create new enemies if number of enemies is less than the max
    if len(enemies) < max_enemies:
        enemy = {
            "x": random.randint(0, screen_width - rect_size[0]),
            "y": random.randint(0, screen_height - rect_size[1]),
            "sizex": rect_size[0],
            "sizey": rect_size[1],
            "color": color
        }
    ...
    for bullet in bullets:
        bullet["x"] += bullet["dir"][1] * bullet_velocity
        bullet["y"] += bullet["dir"][0] * bullet_velocity
        # bullet["x"] += 10

        # check if button has left screen
        if bullet["x"] <= 0 and bullet in bullets:
            bullspeedlist.remove(bullet["dir"])
            bullets.remove(bullet)
        if bullet["x"] >= screen_width and bullet in bullets:
            bullspeedlist.remove(bullet["dir"])
            bullets.remove(bullet)
        if bullet["y"] <= 0 and bullet in bullets:
            bullspeedlist.remove(bullet["dir"])
            bullets.remove(bullet)
        if bullet["y"] >= screen_height and bullet in bullets:
            bullspeedlist.remove(bullet["dir"])
            bullets.remove(bullet)
